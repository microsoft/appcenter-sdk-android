...
import static com.microsoft.appcenter.analytics.Analytics.TRANSMISSION_INTERVAL_MAXIMUM;
import static com.microsoft.appcenter.analytics.Analytics.TRANSMISSION_INTERVAL_MINIMUM;
...
public class AnalyticsTest extends AbstractAnalyticsTest {
    ...
    public void triggerIntervalReturnFalse() {
        ...
        assertFalse(result);
        result = Analytics.setTransmissionInterval(TRANSMISSION_INTERVAL_MAXIMUM + 1 + 1);
        assertFalse(result);
        result = Analytics.setTransmissionInterval(4);
        assertTrue(result);
        result = Analytics.setTransmissionInterval(TRANSMISSION_INTERVAL_MINIMUM - 1);
        assertTrue(result);
        Channel channel = mock(Channel.class);
        analytics.onStarting(mAppCenterHandler);
        analytics.onStarted(mock(Context.class), channel, null, null, false);
