...
public class DistributeBeforeApiSuccessTest extends AbstractDistributeTest {
    ...
    public void doNothingIfReleaseHashEqualsToFailedPackageHash() {
        ...
        doReturn("some_hash").when(DistributeUtils.class);
    ...
    public void continueIfReleaseHashNotEqualsToFailedPackageHash() throws Exception {
        whenNew(Intent.class)
                .withParameterTypes(String.class, Uri.class)
                .withArguments(eq(ACTION_VIEW), any(Uri.class))
                .thenReturn(mock(Intent.class));
        when(SharedPreferencesManager.getString(PREFERENCE_KEY_UPDATE_SETUP_FAILED_PACKAGE_HASH_KEY)).thenReturn("some_hash");
        mockStatic(DistributeUtils.class, CALLS_REAL_METHODS);

        /* Mock the computeReleaseHash to other_hash value. */
        doReturn("other_hash").when(DistributeUtils.class);
        DistributeUtils.computeReleaseHash(any(PackageInfo.class));
